...
function StudyModesPanel({ subjectId }: { subjectId: string }) {
  ...
  const modes = [
    { mode: 'simple-flashcards', title: 'Simple flashcards', text: 'Questions shuffle automatically. Reveal each answer, then move next or undo one step without changing mastery.', icon: <BookOpen /> },
...
type ReviewMode = 'flashcards' | 'simple-flashcards' | 'quick' | 'missed' | 'mixed'

function ReviewPage() {
  const [params] = useSearchParams()
  const subjectId = params.get('subject') ?? ''
  const requestedMode = params.get('mode') as ReviewMode | null
  const mode: ReviewMode = requestedMode && ['flashcards', 'simple-flashcards', 'quick', 'missed', 'mixed'].includes(requestedMode) ? requestedMode : 'flashcards'
  const subject = useLiveQuery(() => subjectId ? db.subjects.get(subjectId) : undefined, [subjectId])
  const liveQuestions = useLiveQuery(() => subjectId ? db.questions.where('subjectId').equals(subjectId).toArray() : Promise.resolve([] as Question[]), [subjectId])
  ...
  const [simpleFlashcardSession, setSimpleFlashcardSession] = useState<{ subjectId: string; questions: Question[] } | null>(null)
  ...
  useEffect(() => {
    if (mode !== 'simple-flashcards' || !liveQuestions?.length) return
    setSimpleFlashcardSession((session) => session?.subjectId === subjectId
      ? session
      : { subjectId, questions: randomSelection(liveQuestions, liveQuestions.length) })
  }, [mode, liveQuestions, subjectId])
  ...
  const pool = mode === 'flashcards'
    ? (flashcardSession ?? [])
    : mode === 'simple-flashcards'
      ? (simpleFlashcardSession?.subjectId === subjectId ? simpleFlashcardSession.questions : [])
      : questions.filter((question) => mode !== 'missed' || question.totalAttempts > question.totalCorrect)
  ...
  function undo() { setIndex((value) => Math.max(0, value - 1)); setRevealed(false) }
  ...
  return <div className="page narrow"><Link className="back-link" to={`/subjects/${subjectId}?tab=review`}><ArrowLeft /> {subject.name} study modes</Link><header className="page-header"><div><p className="eyebrow">{mode === 'flashcards' ? 'Smart flashcards' : mode === 'simple-flashcards' ? 'Simple flashcards' : mode === 'quick' ? 'Quick review' : mode === 'missed' ? 'Missed questions' : 'Mixed test'}</p><h1>Question {index + 1} of {pool.length}</h1></div></header><section className="review-card"><p className="eyebrow">{mode === 'mixed' ? 'Type your answer' : 'Think first, then reveal'}</p><h2>{current.prompt}</h2>{mode === 'mixed' ? <form className="identification-form" onSubmit={checkMixed}><input autoFocus disabled={checked} value={typedAnswer} onChange={(event) => setTypedAnswer(event.target.value)} placeholder="Enter your answer" /><button className="button primary" disabled={checked || !typedAnswer.trim()}>Check answer</button></form> : <>{(revealed || mode === 'quick') && <div className="review-answer"><strong>{current.acceptedAnswers[0]}</strong><p>{current.explanation}</p></div>}{['flashcards', 'simple-flashcards'].includes(mode) && !revealed && <button className="button primary full" onClick={() => setRevealed(true)}>Reveal answer</button>}</>}{checked && <div className={isAcceptedAnswer(typedAnswer, current.acceptedAnswers) ? 'feedback correct' : 'feedback wrong'}><strong>{isAcceptedAnswer(typedAnswer, current.acceptedAnswers) ? 'Correct!' : 'Review this answer'}</strong><p>Answer: <b>{current.acceptedAnswers[0]}</b></p><p>{current.explanation}</p></div>}{mode === 'flashcards' && revealed && <div className="srs-rating-panel"><div><p>How well did you remember?</p><span className={`review-state review-state-${current.reviewState ?? 'new'}`}>{current.reviewState ?? 'new'}</span></div><div className="srs-rating-grid">{(['again', 'hard', 'good', 'easy'] as ReviewRating[]).map((rating) => <button key={rating} className={`srs-rating srs-${rating}`} disabled={savingLevel} onClick={() => void rateFlashcard(rating)}><strong>{rating[0].toUpperCase() + rating.slice(1)}</strong><small>{formatReviewInterval(intervals[rating])}{rating === 'easy' && current.level < 4 ? ` · Tier ${current.level + 1}` : ''}</small></button>)}</div><small className="srs-rating-hint">Again, Hard, and Good keep this card in the activity until you mark it Easy. It returns when due, or immediately after the other queued cards are finished.</small>{levelError && <p className="form-error">{levelError}</p>}</div>}{mode === 'simple-flashcards' && revealed && <div className="simple-flashcard-actions"><button className="button ghost" disabled={index === 0} onClick={undo}><ArrowLeft /> Undo</button><button className="button primary" onClick={next}>{index === pool.length - 1 ? 'Finish review' : 'Next question'} <ChevronRight /></button></div>}{mode === 'flashcards' && <div className="flashcard-card-actions"><button className="button danger" disabled={savingLevel} onClick={() => void deleteCurrentReviewQuestion()}><Trash2 /> Delete this question</button>{!revealed && levelError && <p className="form-error">{levelError}</p>}</div>}{mode === 'quick' && <div className="quick-review-actions"><button className="text-button danger-text" disabled={savingLevel} onClick={() => void deleteCurrentReviewQuestion()}><Trash2 /> Delete</button><button className="button primary" disabled={savingLevel} onClick={next}>{index === pool.length - 1 ? 'Finish review' : 'Next question'} <ChevronRight /></button>{levelError && <p className="form-error">{levelError}</p>}</div>}{mode === 'mixed' && checked && <button className="button primary full" onClick={next}>{index === pool.length - 1 ? 'Finish review' : 'Next question'} <ChevronRight /></button>}</section></div>
}
